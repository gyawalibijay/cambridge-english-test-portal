...
def _published_tests(mode=None):
    ...
    try:
        ...
        if mode and _model_has_field(MockTest, "delivery_mode"):
...
def _test_skill_start_url(skill):
    # B1_READING_50SETS_V36: Reading has its own authoritative Part/Set runner.
    if str(skill).lower() == "reading":
        return "/practice/test/cambridge-reading-reference-practice/"
    for test in _published_tests("practice"):
...
def _featured_mock():
    tests = _published_tests("full_mock")
    required_skills = {"speaking", "listening", "reading", "writing"}
    for test in tests:
        sections = _test_sections(test)
        if not required_skills.issubset({
            str(getattr(section, "skill", "")).lower()
            for section in sections
        }):
            continue
        complete = True
        for section in sections:
            if str(getattr(section, "skill", "")).lower() not in required_skills:
                continue
            parts = list(section.parts.filter(is_active=True))
            if not parts or any(
                not part.question_assignments.filter(question__is_active=True).exists()
                for part in parts
            ):
                complete = False
                break
...
def mock_tests(request):
    ...
    # Keep the Mock Test overview intentionally aligned with the Practice hub.
    # The rows are informational; the full mock still starts as one continuous
    # assessment from the single CTA below them.
    section_by_skill = {
        str(getattr(section, "skill", "")).lower(): section
        for section in sections
    }
    default_parts = {
        "reading": 5,
    ...
    for key, label in (
        ...
        section = section_by_skill.get(key)
        part_count = default_parts[key]
        if section is not None:
            try:
                count = section.parts.filter(is_active=True).count()
                if count:
                    part_count = count
            except Exception:
                try:
                    count = section.parts.count()
                    if count:
                        part_count = count
                except Exception:
                    pass
        mock_skill_rows.append({
            "key": key,
            "label": label,
            "parts": part_count,
        })
    ...
    attempts = [
        attempt for attempt in _attempt_queryset(request.user)
        if getattr(getattr(attempt, "mock_test", None), "delivery_mode", "practice")
        == "full_mock"
    ]
    ...
    context.update({
        ...
        "mock_skill_rows": mock_skill_rows,
