# Cambridge B1 Landing Page — V11 Changelog

## V11 mock-portal showcase

- Adds the requested B1 Mock Test Portal showcase immediately after pricing, preserving the latest uploaded section order.
- Recreates the supplied asymmetric WebVeda card composition with a large Four-Skill Practice card, Exam-Style Experience card, AI Feedback card and wide Level & Progress Check card.
- Uses the supplied Cambridge portal dashboard artwork inside the progress card and authentic bilingual portal copy throughout.
- Connects “Explore Mock Portal” to the existing mock-test route, sending signed-out visitors through the existing login route without changing authentication or mock-test behavior.
- Adds responsive one-column mobile cards, accessible media text, existing reveal motion integration and reduced-motion compatibility.
- No model, migration, configuration, payment, dashboard, practice or assessment logic is changed.

## V10 free-lesson, learner-fit and pricing sequence

- Adds the supplied “Watch a free lesson before you join” section directly after the complete B1 preparation system, using Arya Academy’s Cambridge Upskill complete-guide lesson in a responsive privacy-enhanced YouTube embed.
- Replaces the older separated audience and teaching blocks with the supplied two-panel layout: a clear learner-fit checklist beside the dark personal-guidance card.
- Rebuilds and moves pricing immediately after those panels so the three new sections follow the exact uploaded order without duplicating content or the `#pricing` target.
- Retains the two preparation plans and portal-only option with the current NPR 2,999, NPR 4,999 and NPR 1,999 prices, full supplied feature lists, renewal note and WhatsApp support route.
- Adds desktop, tablet and mobile compositions, accessible headings and media labels, existing reveal motion integration, hover feedback and reduced-motion compatibility.
- Does not change authentication, dashboard, assessment, payment, database or deployment configuration behavior.

## V9 WebVeda hero and conversion-story refinement

- Restores the supplied cinematic studio photograph as one edge-to-edge desktop hero scene, matching WebVeda's integrated background treatment instead of using a visibly separated instructor cut-out.
- Keeps the mobile instructor centred in its own protected image zone and preserves the non-overlapping CTA and three-benefit strip.
- Replaces the CSS-built portal dashboard with the exact supplied portal artwork while retaining accessible, indexable portal copy and the six feature cards.
- Retains the WebVeda-style animated B1 Ready Promise and adds the requested scroll-stacked “Speaking B1 किन अड्किन्छ?” section with five learner problems and a Join Now CTA.
- Adds the requested three-card B1 preparation package, 14-day refund block, class-continuation policy disclosure and complete six-step B1 preparation system.
- Adds responsive sticky-card motion with `prefers-reduced-motion` support and avoids changes to portal, assessment, authentication, payment or database behavior.
...
- Used program-specific promise wording before the refund terms were subsequently supplied for V9.
...
- `static/images/public/cambridge-b1/b1-mock-portal-reference.jpeg` — supplied mock-portal artwork used in the post-accordion portal section.
