(() => {
  ...
  const updateScrollEffects = () => {
    ...
    if (problemCards.length) {
      let currentCard = problemCards[0];
      problemCards.forEach((card, index) => {
        const rect = card.getBoundingClientRect();
        const progress = reducedMotion.matches
          ? 1
          : Math.max(0, Math.min(1, (window.innerHeight * 0.88 - rect.top) / (window.innerHeight * 0.46)));
        card.style.setProperty('--b1x-stack-progress', progress.toFixed(3));
        card.style.setProperty('--b1x-stack-y', ((1 - progress) * 30).toFixed(1) + 'px');
        card.style.setProperty('--b1x-stack-scale', (0.975 + progress * 0.025).toFixed(3));
        const stickyTop = window.innerWidth > 700 ? 104 + index * 17 : 78 + index * 12;
        if (rect.top <= stickyTop + 3) currentCard = card;
      });
      problemCards.forEach(card => card.classList.toggle('is-current', card === currentCard));
    }
