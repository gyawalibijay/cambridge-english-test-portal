...
class PublicHomeTests(TestCase):
    ...
    def test_home_is_the_cambridge_b1_landing_page(self):
        ...
        self.assertContains(response, "Watch a")
        self.assertContains(response, "free lesson")
        self.assertContains(response, "RCNroXsgqFY")
        self.assertContains(response, "This is for you if")
        self.assertContains(response, "You Are Not Just A Student Number")
        self.assertContains(response, "Choose how much")
        self.assertContains(response, "NPR 2,999 is teacher-led preparation")
        self.assertContains(response, "Cambridge Upskill Mock Test Portal")
        self.assertContains(response, "Practise the Test Before")
        self.assertContains(response, "Four-Skill Practice")
        self.assertContains(response, "Exam-Style Experience")
        self.assertContains(response, "AI Feedback")
        self.assertContains(response, "Level &amp; Progress Check")
        self.assertContains(response, "Explore Mock Portal")
        rendered = response.content.decode()
        self.assertLess(rendered.index('class="b1x-free-lesson"'), rendered.index('class="b1x-fit-guidance"'))
        self.assertLess(rendered.index('class="b1x-fit-guidance"'), rendered.index('class="b1x-pricing b1x-pricing-v10"'))
        self.assertLess(rendered.index('class="b1x-pricing b1x-pricing-v10"'), rendered.index('class="b1x-mock-showcase"'))
        self.assertLess(rendered.index('class="b1x-mock-showcase"'), rendered.index('class="b1x-comparison"'))
