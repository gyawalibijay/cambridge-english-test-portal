...
CAMBRIDGE_PUBLIC_KEY = "cambridge-english"


def _render_cambridge_home(request):
    landing = (
        PublicProgramLanding.objects
        .filter(program_key=CAMBRIDGE_PUBLIC_KEY, is_active=True)
        .first()
    )
    if landing is None:
        raise Http404("Cambridge preparation page not found.")

    return render(
        request,
        "website/cambridge_b1_landing.html",
        {
...
def home(request):
    """Serve Cambridge/B1 as the site's single public product homepage."""
    return _render_cambridge_home(request)


def public_program_landing(request, program_key):
    """Keep former marketing URLs working without exposing retired products."""
    return redirect("public_home", permanent=True)
